...
use whitelist_hide_core::compiler::{EngineFlavor, compile_strategy};
...
fn strategy_compile(path: &Path, engine: &str) -> i32 {
    ...
    let engine = match EngineFlavor::parse(engine) {
        Ok(engine) => engine,
        Err(error) => {
            eprintln!("engine: INVALID");
            eprintln!("reason: {error}");
            return 2;
        }
    };

    match compile_strategy(&strategy, path, engine) {
        Ok(compiled) => {
